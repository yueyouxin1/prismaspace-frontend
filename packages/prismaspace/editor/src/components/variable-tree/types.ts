import type { SchemaType } from "../param-schema-editor/core";
...
export interface ResolvedVariableTreeNodeValue {
  label: string;
  nextLabels: string[];
  blockID: string;
  path: string;
...
export interface DefaultVariableTreeItem {
  key: string;
  label: string;
  caption: string;
  source?: string;
  ...
  resolved: ResolvedVariableTreeNodeValue;
  children: DefaultVariableTreeItem[];
}

export interface FlatDefaultVariableTreeItem extends DefaultVariableTreeItem {
  level: number;
  hasChildren: boolean;
  isLeaf: boolean;
}

export type VariableTreeInsertValueResolver = (
  node: VariableTreeNode,
  resolved: ResolvedVariableTreeNodeValue,
) => string | null;
